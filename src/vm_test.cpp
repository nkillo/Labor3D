...
void init_opcode_lookup(VM* vm) {
    ...
    vm->opLookupTable[GEN_JMP][ADDR_LABEL][ADDR_NONE][ADDR_NONE] = OP_JMP_CONSTANT;
...
inline void vmMemError(VM& vm, char* message, u32 instructionLocation, s32 memLocation, u32 maxMem) {
    printf("[VM ERROR]: %s, instruction: %u, memory address: %d, max memory size: %u\n", message, instructionLocation, memLocation, maxMem);
}

inline void vmError(VM& vm, char* message, u32 instructionLocation) {
    printf("[VM ERROR]: %s, instruction: %u\n", message, instructionLocation);
}

inline bool executeInstruction(VM& vm) {
    u32 currentByte = vm.pc;
    if (vm.pc >= vm.byteCount) {
        printf("program counter: %u, exceeds byteCount: %u, returning\n", vm.pc, vm.byteCount);
        return true;
    }
    ...
    case OP_HLT: {
        printf("%2u: HLT ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        return true;
    }break;
    case OP_IGL: {
        printf("%2u: IGL ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        return true;
    }break;
    case OP_LOAD_REG_TO_REG: {
        printf("%2u: LOAD REG2REG ENCOUNTERED at pc %u   :    ", currentByte, vm.pc - 1);
    ...
    case OP_LOAD_IMM_TO_REG: {
        printf("%2u: LOAD ENCOUNTERED at pc %2u   :    ", currentByte, vm.pc - 1);
    ...
    case OP_ADD_REG_TO_REG: {
        printf("%2u: ADD ENCOUNTERED at pc %2u    :    ", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
    ...
    case OP_SUB_REG_TO_REG: {
        printf("%2u: SUB ENCOUNTERED at pc %2u   :    ", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        u8 destreg = nextByte(vm);
        printf("SUB $%u $%u $%u, \t %u - %u = %u\n", reg1, reg2, destreg, vm.registers[reg1], vm.registers[reg2], vm.registers[reg1] - vm.registers[reg2]);
        vm.registers[destreg] = vm.registers[reg1] - vm.registers[reg2];
        return false;

    }break;
    case OP_MUL_REG_TO_REG: {
        printf("%2u: MUL ENCOUNTERED at pc %u  :     ", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        u8 destreg = nextByte(vm);
        printf("MUL $%u $%u $%u, \t %u * %u = %u\n", reg1, reg2, destreg, vm.registers[reg1], vm.registers[reg2], vm.registers[reg1] * vm.registers[reg2]);
        vm.registers[destreg] = vm.registers[reg1] * vm.registers[reg2];
        return false;

    }break;
    case OP_DIV_REG_TO_REG: {
        printf("%2u: DIV ENCOUNTERED at pc %u   :    ", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        u8 destreg = nextByte(vm);
    ...
    case OP_JMPF: {
        printf("%2u: JMPF ENCOUNTERED at pc %u, jumpCount: %u\n", currentByte, vm.pc - 1, vm.jumpCount + 1);
        u8 reg = nextByte(vm);
    ...
    case OP_JMPB: {
        printf("%2u: JMPB ENCOUNTERED at pc %u, jumpCount: %u\n", currentByte, vm.pc - 1, vm.jumpCount + 1);
        u8 reg = nextByte(vm);
        vm.pc -= vm.registers[reg] + 2;// - 2 to account for the first 2 instructions we've already executed
        printf("JMPB %u\n", vm.registers[reg]);
        vm.jumpCount++;
    ...
    case OP_EQ: {
        printf("%2u: EQ ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
    ...
    case OP_NEQ: {
        printf("%2u: NEQ ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        if (vm.registers[reg1] != vm.registers[reg2])vm.equalFlag = true;
        else vm.equalFlag = false;
    ...
    case OP_GT: {
        printf("%2u: GT ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        if (vm.registers[reg1] > vm.registers[reg2])vm.equalFlag = true;
        else vm.equalFlag = false;
    ...
    case OP_LT: {
        printf("%2u: LT ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        if (vm.registers[reg1] < vm.registers[reg2])vm.equalFlag = true;
        else vm.equalFlag = false;
    ...
    case OP_GTQ: {
        printf("%2u: GTQ ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        if (vm.registers[reg1] >= vm.registers[reg2])vm.equalFlag = true;
        else vm.equalFlag = false;
    ...
    case OP_LTQ: {
        printf("%2u: LTQ ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        if (vm.registers[reg1] <= vm.registers[reg2])vm.equalFlag = true;
        else vm.equalFlag = false;
    ...
    case OP_JEQ_REG: {
        printf("%2u: JEQ ENCOUNTERED at pc %u, jumpCount: %u   :   ", currentByte, vm.pc - 1, vm.jumpCount);
        u8 reg1 = nextByte(vm);
        s32 target = vm.registers[reg1];
        printf("JEQ %u, equalFlag: %d\n", target, vm.equalFlag);
        if (vm.equalFlag) {
            vm.equalFlag = false;
            vm.jumpCount++;
    ...
    case OP_INC: {
        ...
        vm.registers[reg1]++;
        printf("%2u: INC ENCOUNTERED at pc %u, vm.registers[$%u] is now %d\n", currentByte, currentByte, reg1, vm.registers[reg1]);
        vm.pc += 2;//need to pad out to the next instruction
        return false;
    }break;
    case OP_DEC: {
        ...
        Assert(reg1 >= 0 && reg1 < 32);
        vm.registers[reg1]--;
        printf("%2u: DEC ENCOUNTERED at pc %u, vm.registers[$%u] is now %d\n", currentByte, currentByte, reg1, vm.registers[reg1]);
        vm.pc += 2;//need to pad out to the next instruction
        return false;
    }break;
    ...
    case OP_LOAD_REG_ADDR_TO_OFFSET_REG_ADDR: {
        printf("%2u: LOAD REG ADDR TO OFFSET REG ADDR ENCOUNTERED at pc %u :    ", currentByte, vm.pc - 1);
    ...
    case OP_LOAD_OFFSET_REG_ADDR_TO_REG: {
        printf("%2u: LOAD OFFSET REG ADDR TO REG ENCOUNTERED at pc %u :    ", currentByte, vm.pc - 1);
    ...
    case OP_LOAD_OFFSET_REG_ADDR_TO_REG_ADDR: {
        printf("%2u: LOAD OFFSET REG ADDR TO REG ADDR ENCOUNTERED at pc %u :    ", currentByte, vm.pc - 1);
        u8 reg1 = vm.bytecode[vm.pc++];
        u8 reg2 = vm.bytecode[vm.pc++];
        u8 offset = vm.bytecode[vm.pc++];
    ...
    case OP_LOAD_DATA_ADDR_TO_ADDR: {
        // __debugbreak();
        printf("%2u: LOAD DATA ADDRESS TO ADDRESS ENCOUNTERED at pc %u   :    ", currentByte, vm.pc - 1);
        ...
        printf("LOAD DATA ADDRESS TO ADDRESS [$%u] [$%u]\n", reg1, reg2);
        return false;
    }break;

    case OP_JMP_CONSTANT: {
        // __debugbreak();
        printf("%2u: JMP CONSTANT ENCOUNTERED at pc %u   :    ", currentByte, vm.pc - 1);
    ...
    case OP_JEQ_REG_TO_REG_CONSTANT: {
        // __debugbreak();
        printf("%2u: JEQ REG TO REG ENCOUNTERED at pc %u, jumpCount: %u   :    ", currentByte, vm.pc - 1, vm.jumpCount);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
    ...
    case OP_EQ_CONST_TO_REG: {
        printf("%2u: OP_EQ_CONST_TO_REG ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u16 val = next2Bytes(vm);
    ...
    case OP_EQ_INDIRECT_REG_TO_REG: {
        printf("%2u: OP_EQ_INDIRECT_REG_TO_REG ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
        u8 reg1 = nextByte(vm);
        u8 reg2 = nextByte(vm);
        s32 val1 = vm.registers[reg1];
    ...
    case OP_PRT_ADDRESS: {
        // __debugbreak();
        printf("%2u: PRT ADDRESS ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
    ...
    case OP_PRT_REG: {
        // __debugbreak();
        printf("%2u: PRT REG ENCOUNTERED at pc %u\n", currentByte, vm.pc - 1);
    ...
    case OP_PUSH_REG: {
        ...
        *((s32*)(vm.mem + vm.registers[REGSP])) = vm.registers[reg1];
        ...
        printf("%2u: PUSH ENCOUNTERED at pc %u, pushed %d onto stack\n", currentByte, currentByte, vm.registers[reg1]);
    ...
    case OP_POP_REG: {
        ...
        printf("%2u: POP ENCOUNTERED at pc %u, popped %d onto $%u\n", currentByte, currentByte, vm.registers[reg1], reg1);
    ...
    case OP_CALL: {
        printf("%2u: CALL ENCOUNTERED at pc %u\n", currentByte, currentByte);
    ...
    case OP_RET: {
        printf("%2u: RET ENCOUNTERED at pc %u\n", currentByte, currentByte);
    ...
    case OP_SYSCALL: {
        printf("%2u: SYSCALL ENCOUNTERED at pc %u\n", currentByte, currentByte);
...
u8 lookup_opcode(VM* vm, u8 operation, addressing_mode arg1, addressing_mode arg2, addressing_mode arg3) {
    ...
    return 255;
#else //this is in a function in case we later want to use a hash map instead of a retarded 4D table
...
inline void parseLOAD(VM* vm, Parser* parser, Scanner* scanner, Opcode code) {
    ...
    if (inst.final_opcode == 255) {
        errorAt(parser, &instructionToken, "invalid addressing mode combination");
        return;
    }
...
int repl_command(REPL* repl) {
    ...
    case '/': {//repl command
        ...
        if (scanner->current - scanner->start > 1) {
            switch (scanner->start[0]) {
            case 'c': {
                if (checkReplKeyword(scanner, 1, 3, "lear")) {
                    printf("clearing bytecode and bytecount!\n");
                    repl->vm.byteCount = 0;
                    return 0;
                }
            }break;
...
void test_label_data(REPL* repl) {
    ...
    Assert(repl->vm.registers[0] == 4); //just to make it pass
}
...
void test_repl_reg_val(const char* command, u32 reg, int val) {
    ...
    REPL* repl = (REPL*)plat_alloc_mem(sizeof(REPL));
    ...
    plat_dealloc_mem(repl, sizeof(REPL));
...
void vm_test() {
    ...
    VM* vm = (VM*)plat_alloc_mem(mem_size);
    reset_vm(vm);
